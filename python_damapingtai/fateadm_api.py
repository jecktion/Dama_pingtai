...
class Rsp():
    ...
    def ParseJsonRsp(self, rsp_data):
        ...
        if rsp_data is None:
            ...
            return
        jrsp                = json.loads(str(rsp_data,encoding='utf-8'))
...
def CalcSign(usr_id, passwd, timestamp):
    ...
    md5     = hashlib.md5()
    md5.update((usr_id + timestamp + csign).encode("utf8"))
    csign   = md5.hexdigest()
    return csign

def CalcCardSign(cardid, cardkey, timestamp, passwd):
    md5     = hashlib.md5()
    md5.update((passwd + timestamp + cardid + cardkey).encode("utf8"))
    return md5.hexdigest()

def HttpRequest(url, body_data):
    rsp         = Rsp()
    post_data   = urllib.parse.urlencode(body_data).encode(encoding='UTF8')
